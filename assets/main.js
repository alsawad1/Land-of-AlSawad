...
const SOCIAL_LINKS = [
    { name: 'Instagram', url: 'https://www.instagram.com/alsawad.1qr/', icon: '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M12 2.16c3.2 0 3.58.01 4.85.07 3.25.15 4.77 1.69 4.92 4.92.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.15 3.23-1.66 4.77-4.92 4.92-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-3.26-.15-4.77-1.7-4.92-4.92-.06-1.27-.07-1.65-.07-4.85s.01-3.58.07-4.85C2.38 3.92 3.9 2.38 7.15 2.23 8.42 2.17 8.8 2.16 12 2.16zm0 5.84a4 4 0 1 0 0 8 4 4 0 0 0 0-8zm0 6.6a2.6 2.6 0 1 1 0-5.2 2.6 2.6 0 0 1 0 5.2zm5.2-6.76a.94.94 0 1 1-1.87 0 .94.94 0 0 1 1.87 0z"/></svg>' },
...
function injectSocialIcons() {
    ...
    if (!footerEl || document.querySelector('.social-icons')) return;
    const wrap = document.createElement('div');
    wrap.className = 'social-icons';
    wrap.innerHTML = SOCIAL_LINKS.map(s =>
        `<a href="${s.url}" target="_blank" rel="noopener" aria-label="${s.name}" class="social-icon-link">${s.icon}</a>`
    ).join('');
    footerEl.parentNode.insertBefore(wrap, footerEl);
}
